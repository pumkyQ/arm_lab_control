...
const int JOY_X_PIN = A0;  // JRX
const int JOY_Y_PIN = A1;  // JRY
...
void handleJoystickInputs() {
  ...
  // 1. 조이스틱 좌/우 제스처 감지
  if (joyX < 250) { // 🕹️ 조이스틱 왼쪽 (S: 손가락 굽힘 시퀀스)
    if (!joyTriggered) {
      joyTriggered = true;
      if (currentState == STANDBY || currentState == HOLD) {
        currentState = ALIGN;
        stateStartTime = millis();
        Serial.println(F("\n🕹️ [조이스틱 왼쪽 감지] 굽힘 시퀀스(S) 시작 (0° 정렬 중...)"));
      }
    }
  } else if (joyX > 750) { // 🕹️ 조이스틱 오른쪽 (E: J1 ±30° 3회 왕복)
    if (!joyTriggered) {
      joyTriggered = true;
      currentState = WIGGLE_J1;
      stateStartTime = millis();
      Serial.println(F("\n🕹️ [조이스틱 오른쪽 감지] J1 관절 왕복 운동(E) 시작!"));
    }
  } else if (joyX >= 350 && joyX <= 650) {
    joyTriggered = false; // 중앙 복귀 시 트리거 플래그 해제
  }

  // 2. 조이스틱 푸시 스위치 감지 (SW 스위치 눌림 -> R: 원점 리셋)
  if (swBtn == LOW) {
    if (!btnTriggered) {
      btnTriggered = true;
      ...
      Serial.println(F("\n🕹️ [조이스틱 버튼 감지] 대기 상태 복귀 및 원점 리셋(R)"));
    }
  } else {
    btnTriggered = false;
  }
...
void setup() {
  ...
  // 조이스틱 SW 버튼 핀 입력 설정 (내부 풀업 저항 활성화)
  pinMode(JOY_SW_PIN, INPUT_PULLUP);

  Serial.println(F("========================================================="));
  Serial.println(F(" 🎯 KITECH 4축 로봇 핑거 아두이노 조이스틱 제어기"));
  Serial.println(F("========================================================="));
  ...
  Serial.println(F("▶ 조이스틱 조작 안내:"));
  Serial.println(F("  [조이스틱  왼쪽] 👈 : 굽힘 시퀀스 (S) 실행"));
  Serial.println(F("  [조이스틱 오른쪽] 👉 : J1 관절 (-30° <-> +30°) 3회 왕복 (E) 실행"));
