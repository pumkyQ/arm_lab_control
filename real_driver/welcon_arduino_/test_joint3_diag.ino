...
// =========================================================================
// 🎯 Welcon 조인트 3번 (Node 2) 정밀 고장 및 원인 정밀 진단 전용 아두이노 스케치
// =========================================================================
const int SPI_CS_PIN = 10;
MCP_CAN CAN(SPI_CS_PIN);

const uint8_t TARGET_NODE = 2; // 조인트 3번 Node ID = 2
...
void diagnoseAxis(uint8_t nodeId, uint8_t axis) {
  ...
  Serial.print(F(" - Axis ")); Serial.println(axis);
  Serial.println(F("---------------------------------------------------------"));
...
void setup() {
  ...
  Serial.println(F("========================================================="));
  Serial.println(F(" 🔬 Welcon Node 2 (조인트 3번) 모터/드라이버 정밀 진단 툴"));
  Serial.println(F("========================================================="));
...
void loop() {
  // 1초마다 엔코더 및 상태 실시간 모니터링
  static unsigned long lastT = 0;
  if (millis() - lastT >= 1000) {
    lastT = millis();

    bool ok1 = false, ok2 = false;
    int32_t pos1 = readSdoInt32(2, 0x6064, 0x00, ok1);
    int32_t pos2 = readSdoInt32(2, 0x6864, 0x00, ok2);
    int32_t err1 = readSdoInt32(2, 0x603F, 0x00, ok1);

    Serial.print(F("📍 Node 2 Realtime -> Axis 1 Pos: "));
    Serial.print(pos1);
    Serial.print(F(" | Axis 2 Pos: "));
    Serial.print(pos2);
    Serial.print(F(" | Err1: 0x"));
