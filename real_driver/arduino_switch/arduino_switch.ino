...
JointConfig joints[4] = {
  // j1: Node 3, Axis 1 (Ki 조절값 반영: 0.5f, Ki_limit: 300.0f)
  ...
  // j4: Node 3, Axis 2 (새 모터 계산 원점 385.0f 반영)
  {"j4", 3, 2,  385.0f, 1437.0f,  -611.0f, 350.0f, 15.0f, 1.5f, 1000.0f, 3.0f, 1000.0f}
};
...
void readJointFeedbacks() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // 2. 상태 워드 200ms 주기 읽기 및 모터 드라이버 가동 상태(Operation Enable) 자동 지속 복구
    if (pollStatus) {
      bool statOk = false;
      ...
      if (statOk) {
        ...
        // Fault(0x08) 상태이거나 Operation Enable(bit2=1 및 Switched On) 상태가 아닐 때 재활성화
...
void handleSwitchInputs() {
  ...
  if (btnBend == LOW) {
    if (!sw6Trig && (now - lastBtn6Time > 50)) {
      ...
      stateStartTime = now;
      Serial.println(F("\n🔘 [스위치 2 (D6) 감지] j2~j4 손가락 굽힘 시퀀스 시작!"));
    }
  } else {
  ...
  if (btnWiggle == LOW) {
    if (!sw7Trig && (now - lastBtn7Time > 50)) {
      ...
      stateStartTime = now;
      Serial.println(F("\n🔘 [스위치 3 (D7) 감지] 조인트 1번 (-70° <-> +70°) 3회 왕복 운동 시작!"));
    }
  } else {
...
void setup() {
  ...
  Serial.println(F("▶ 하드웨어 스위치 조작 안내:"));
  Serial.println(F("  [스위치 1 (D5)] 🔘 : '정렬' (4개 관절 모두 0° 복귀)"));
  Serial.println(F("  [스위치 2 (D6)] 🔘 : '조인트 2~4번 굽히기' (20° -> 40° -> 35° 순차 구동)"));
  Serial.println(F("  [스위치 3 (D7)] 🔘 : '조인트 1번 ±70° 3회 왕복' (j2~j4 0° 고정)"));
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (cmd == 's' || cmd == 'S') {
      if (currentState == STANDBY || currentState == HOLD || currentState == ALIGN_ONLY) {
        currentState = BEND_SEQ;
        stateStartTime = millis();
        Serial.println(F("\n🔥 [시퀀스 시작] j2, j3, j4 정렬 후 굽힘 시작 (0° 정렬 중...)"));
      }
    } else if (cmd == 'e' || cmd == 'E') {
      currentState = WIGGLE_J1;
      stateStartTime = millis();
